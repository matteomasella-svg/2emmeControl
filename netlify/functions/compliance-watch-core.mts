...
export async function runComplianceWatch(options:{deep?:boolean;sourceLimit?:number;reason?:string}={}){
  ...
  const now=new Date().toISOString(); const results:any[]=[]; const sourcePatches:any[]=[]; const newUpdates:any[]=[];
  const active=sourceRows.filter((r:any)=>Boolean(r.fields?.[SF.active])).slice(0,options.sourceLimit||50);
  for(const src of active){ const f=src.fields||{}; const root=String(f[SF.url]||''); if(!root){results.push({source:f[SF.id]||src.id,status:'SKIPPED',error:'URL_MISSING'});continue;} const keywords=keywordList(f[SF.keywords]); try{
      ...
      if(options.deep){ const links=extractRelevantLinks(rootRes.text,rootRes.url,keywords,4); for(const link of links){ try{ const sub=await fetchText(link,4200); pages.push({url:sub.url,text:sub.text}); }catch{} } }
      ...
      if(changed && !existingHashes.has(contentHash)){
        const updateId=`UPD-${String(f[SF.id]||src.id).replace(/[^A-Za-z0-9-]/g,'-')}-${now.replace(/[-:.TZ]/g,'').slice(0,14)}`;
      ...
      results.push({source:f[SF.id]||src.id,name:f[SF.name]||'',status:baseline?'BASELINE':changed?'CHANGED':'UNCHANGED',hash:contentHash,pages:pages.length,url:rootRes.url});
    }catch(err:any){ results.push({source:f[SF.id]||src.id,name:f[SF.name]||'',status:'ERROR',error:String(err?.message||err),url:root}); sourcePatches.push({id:src.id,fields:{[SF.checked]:now}}); }
  }
  ...
  return {ok:true,scanned_at:now,deep:Boolean(options.deep),sources_scanned:active.length,changes_detected:newUpdates.length,results};
}
